...
(function ($w) {
  ...
  jm.view_provider.prototype = {
    ...
    init_canvas: function () {
      ...
      this.createAddHandlerDOMNode();
    },

    createToolsDOMNode: function () {
      if (!this.jm.options.editable) {
        return;
      }
          ...
          <Space>
            ...
            <Tooltip content="放大">
              <Button
                ...
                icon={<IconZoomIn />}
                onClick={() => {
                  this.jm.view.zoomIn(true);
                }}
              />
            </Tooltip>
            <Divider />
            <Tooltip content="新增节点">
              <Button
                size="small"
                type="tertiary"
                theme="borderless"
      ...
      this.container.parentElement.appendChild(div);
    },
    ...
    showAddHandlerDOMNode: function () {
      const el = this._add_handler_dom_node;

      if (!this.jm.options.editable) {
        this.hideAddHandlerDOMNode();
        return;
      }
      ...
      el.style.left =
        direction === -1 ? `${node._data.view.abs_x - 34}px` : `${node._data.view.abs_x + node._data.view.width + 5}px`;
    },
  ...
  jm.shortcut_provider.prototype = {
    ...
    handler: function (e) {
      ...
      // const isClickInsideElement = this.jm.view.container.contains(evt.target);
    ...
    handle_delnode: function (_jm, e) {
      ...
      if (!!selected_node && !selected_node.isroot) {
        _jm.select_node(selected_node.parent);
        _jm.remove_node(selected_node);
        this.view.showAddHandlerDOMNode();
      }
    },
